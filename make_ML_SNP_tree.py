...
def main():
	...
	# Find ML topology
	find_ML_topology = '-s %s -n topology -# 500 -p 65432 -m ASC_GTRGAMMA --asc-corr=lewis -t init_tree.nwk' % phylip
	...
	run_PhyML(phylip, base, '0', outpath, freqs, kappa, alpha,
		'BEST', '-u RAxML_bestTree.topology -m GTR -o lr --quiet --sequential')
	...
	generate_bootstrap_trees = '-n boots -s %s -N %s -f a -m ASC_GTRGAMMA --asc-corr=lewis -p 65432 -x 54321' % (phylip, opts.boots)
	run_RAxML(raxml, cpus, generate_bootstrap_trees, outpath, base)
	
	# Project boostrap values onto ML topology
	project_boots_onto_ML_topology = '-n final -t %s.ML_topol.nwk -z RAxML_bootstrap.boots -f b -m ASC_GTRGAMMA --asc-corr=lewis' % base
